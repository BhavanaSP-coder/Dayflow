...
async def mark_notification_read(
    db: AsyncSession, current_user: CurrentUser, notification_id: int
) -> MarkReadOut:
...
async def get_unread_count(db: AsyncSession, current_user: CurrentUser) -> dict:
    return {"unread": await notification_service.count_unread(db, user_id=current_user.id)}
