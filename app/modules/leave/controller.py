...
def apply_for_leave(
    db: Session, current_user: CurrentUser, payload: LeaveCreate
) -> LeaveActionOut:
    ...
    leave = leave_service.create_leave(
        db, employee_id=current_user.employee_id, payload=payload
    )
...
def list_my_leaves(
    ...
    items, total = leave_service.get_my_leaves(
        db, employee_id=current_user.employee_id, status=status, skip=skip, limit=limit
    )
    return LeaveListOut(total=total, items=[LeaveOut.model_validate(i) for i in items])


def get_leave(db: Session, current_user: CurrentUser, leave_id: int) -> LeaveOut:
    leave = leave_service.get_leave_for_user(
...
def list_all_leaves(
    ...
    items, total = leave_service.list_all_leaves(
...
def approve_leave(
    db: Session, current_user: CurrentUser, leave_id: int, payload: LeaveDecision
) -> LeaveActionOut:
